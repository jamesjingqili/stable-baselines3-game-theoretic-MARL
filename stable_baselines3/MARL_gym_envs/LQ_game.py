...
class LQ_Game_Env(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        state_cost = np.zeros((4, 4))
        state_cost[0,0] = 1
        state_cost[1,1] = 1
        for i in range(self.num_players):
            if i == 0:
                # import pdb; pdb.set_trace()
                self.Q[i*self.nx:(i+1)*self.nx, i*self.nx:(i+1)*self.nx, i] = state_cost #np.eye(self.nx)
        ...
        self.state_high = np.ones((self.state_space_dim,)) * bound
        self.state_low = -self.state_high
        self.action_high = np.ones((self.action_space_dim,)) * bound
    ...
    def step(self, u):
        ...
        self.costs = -np.array([
            ...
            1/2 * U.T  @ self.R[:,:,i] @ U).item() for i in range(self.num_players) # @ self.R[:,:,i] 
        ])
...
class LQ_Game_Env1(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        for i in range(self.num_players):
            if i == 0:
                # import pdb; pdb.set_trace()
                self.Q[i*self.nx:(i+1)*self.nx, i*self.nx:(i+1)*self.nx, i] = state_cost
                self.R[i*self.nu:(i+1)*self.nu, i*self.nu:(i+1)*self.nu, i] = np.eye(self.nu)
            elif i == 1:
                self.Q[i*self.nx:(i+1)*self.nx, i*self.nx:(i+1)*self.nx, i] = state_cost
                self.R[i*self.nu:(i+1)*self.nu, i*self.nu:(i+1)*self.nu, i] = np.eye(self.nu)
    ...
    def step(self, u):
        ...
        # update cost for each player
        self.costs = (-np.array([
            # (1/2 * next_X.T @ self.Q[:,:,i]   @ next_X + # @ self.Q[:,:,i] 
